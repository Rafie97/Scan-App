      ...
      try {
        qSnap.forEach(async (doc, index) => {
          ...
          if (this.state.scannedItems.find(e => (e.docID === item.docID))) {
            ...
            await this.setState({ scannedItems: [...this.state.scannedItems, item], loading: false })
          }
        })
      }
      catch (e) {
    ...
    return (
      <ImageBackground source={require('../res/android-promotions.png')} style={styles.fullBackground} >
        <View style={styles.scanContainer} >
          ...
          <RNCamera ref={ref => { this.camera = ref; }} style={styles.cameraWindow} ratio="1:1" onGoogleVisionBarcodesDetected={this.barcodeRecognized} autoFocus="on" captureAudio={false}  >
          </RNCamera>
          <View style={styles.underCam}>

            {
              (this.state.loading && this.state.scannedItems.length == 0) ? (<Text style={styles.scannedItemsText}>Please scan a barcode to begin</Text>) :
                (<View style={styles.underCam}>
                  <Text style={styles.scannedItemsText}>Scanned Items</Text>
                  <FlatList data={this.state.scannedItems} keyExtractor={(item, index) => index.toString()} renderItem={this.renderItem}/>
                </View>
                )
            }
